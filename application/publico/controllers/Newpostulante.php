...
class Newpostulante extends Controladoradmin {
    ...
    function __construct() {
        ...
        $this->carpeta = 'newpostulante/';
        $this->controlador = 'newpostulante/';
...
//        $this->sector_exp[-1] = 'Seleccione el Sector de Experiencia';
    ...
    function aviso() {
        ...
        $this->load->view('postulante/aviso', $contenido);
    }
...
//            echo $passconfirm."<br>";
                ...
                if (@$error_acuerdo || @$error_telefono || @$error_pass) {
                    ...
                    if ($this->input->post($prefijo . 'traslado')) {
                        $data[$prefijo . 'traslado'] = $this->input->post($prefijo . 'traslado');
                    ...
                    if ($this->input->post($this->prefijoF . 'salario')) {
                        $dataF[$this->prefijoF . 'salario'] = $this->input->post($this->prefijoF . 'salario');
                    } else {
                        $dataF[$this->prefijoF . 'salario'] = 0;
                    }
                    // var_dump($fila[$prefijo . 'traslado']);
                    // exit;
...
//                para colocar al postulante en disponibilidad inmediata
                    ...
                    if ($id && $idF) {
                        ...
                        redirect('newpostulante/editar_experiencia_sintesis1');
                        // redirect('convocatoria/postular');
                    }
                }
            }
        } else {
            redirect('newpostulante/existepostulante');
        }
    }

    function existepostulante() {
        $formulario = 'existe_postulante';
        $ci = $_SESSION[$this->presession . 'ci'];
        $consulta = $this->db->query('
        SELECT ' . $this->prefijo . 'documento,' . $this->prefijo . 'id FROM ' . $this->tabla . ' where ' . $this->prefijo . 'documento="' . $ci . '"');
       
        $fila = $consulta->row_array();

        $contenido['id'] = $fila[$this->prefijo . 'id'];
        $data['contenido'] = $this->load->view($this->carpeta . $formulario, $contenido, true);
...
//        $id;
    ...
    function editar_datospersonale() {
...
//        $id;
        ...
        $this->cabecera['accion'] = 'Editar Datos Personales';
        $contenido['cabecera'] = $this->cabecera;
        $contenido['action'] = $this->controlador . 'guardar_editar_datospersonale';
        $contenido['fila'] = $fila;
        $contenido['paises'] = $newPaises;
        $contenido['ciudades'] = $newCiudades;
        $data['contenido'] = $this->load->view($this->carpeta . $formulario, $contenido, true);
...
//                para modificar la fecha de la edicion
    ...
    function guardar_editar_datospersonale() {

        $this->boton_actual = 'Datos Personales';
        $formulario = 'datospersonales_editar_form';
        $campo = array($this->prefijo . 'nombre', $this->prefijo . 'apaterno', $this->prefijo . 'amaterno', $this->prefijo . 'nacionalidad',
            $this->prefijo . 'direccion', $this->prefijo . 'telefono', $this->prefijo . 'celular', $this->prefijo . 'email', $this->prefijo . 'traslado');
        $campoF = array($this->prefijoF . 'sexo', $this->prefijoF . 'fecha_nacimiento', $this->prefijoF . 'pais_ciudad', $this->prefijoF . 'ciudad_otra', $this->prefijoP . 'id', 'ciu_id');
...
//                para modificar la fecha de la edicion
                ...
                if ($this->$modelo->editar($data) && $this->$modelo->editarF($dataF)) {
//                    echo 'aaa';
                    redirect('convocatoria/cvtemporal');
                }
            }
        }
    }

    function editar_experiencia_sintesis1() {
        $this->boton_actual = 'Trayectoria Laboral';
        $formulario = 'experiencia_sintesis_form1';
...
//                para actulizar la fecha de uno de los tab
                ...
                // redirect('newpostulante/trayectoria_laboral');
                redirect('convocatoria/postular');
            }
        }
    }
    ...
    function postgrado_nuevo() {
        ...
        if ($this->input->post('sin_postgrado')) {
            redirect('newpostulante/superior_nuevo');
        } else {
...
//                para actulizar la fecha de uno de los tab
    ...
    function superior_nuevo() {
        ...
        if ($this->input->post('sin_superior')) {
            redirect('postulante/secundaria_nuevo');
        } else {
...
//        fin
        ...
        $this->campos_listar_trayectoria = array('Desde', 'Hasta', 'Nombre Organización', 'País/ciudad', 'Cargo(s) Ocupado(s)', 'Sueldo por Mes', 'Teléfono Organización', 'Nombre, Telf., Email Inmediato Superios');
    ...
    function trayectoria_nuevo() {
        ...
        if ($this->input->post('sin_trayectoria')) {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                for ($i = 0; $i < count($campo); $i++) {
                    if (($campo[$i] == 'tra_actual') && ($this->input->post($campo[$i]) == Null)) {
                        $data[$campo[$i]] = 0;
                    } else {
                        $data[$campo[$i]] = $this->input->post($campo[$i]);
                    }
...
//                para actulizar la fecha de uno de los tab
                ...
                redirect('postulacion/agregarnuevo');
//                exit();
//                redirect('postulante/pcompleta');
            }
        }
    }
    ...
    function idioma_nuevo() {
        ...
        if ($this->input->post('sin_idioma')) {
            redirect('postulante/informacion_adicional');
        } else {
...
//                para actulizar la fecha de uno de los tab
                    $this->Ufecha('fecha_informacion_adicional');
                    redirect('newpostulante/informacion_adicional');
                }
            }
        }
    }
...
//                'rules' => 'is_numeric'
            ...
            array(
                'field' => $prefijo . 'pass',
                'label' => 'Contraseña',
                'rules' => 'required|min_length[8]|max_length[30]|valid_password'
            ),
            array(
                'field' => $prefijo . 'pass1',
