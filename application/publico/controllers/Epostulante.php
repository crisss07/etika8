...
class Epostulante extends Controladoradmin {
...
//****** definiendo nombre de carpeta por defecto
        $this->carpeta = 'epostulante/';
        $this->controlador = 'epostulante/';
...
//        $this->sector_exp[-1] = 'Seleccione el Sector de Experiencia';
    ...
    function aviso() {
        ...
        $this->load->view('epostulante/aviso', $contenido);
    }
...
//                para colocar al postulante en disponibilidad inmediata
                ...
                if ($id && $idF) {
                    ...
                    redirect('epostulante/instruccion_formal');
                }
            }
        }
    }
...
//        fecha trayectorias
        $consultaFecha = $this->db->query('
        SELECT date(' . $this->prefijo . 'fecha_edicion) as fecha
        FROM ' . $this->tabla . '
        WHERE ' . $this->prefijo . 'id=' . $id . '
        ORDER BY
        ' . $this->prefijo . 'fecha_edicion desc limit 1'
        );
//        $consultaFecha = $this->db->query('
...
//                para actulizar la fecha de uno de los tab
                ...
                redirect('epostulante/trayectoria_laboral');
            }
        }
    }
    ...
    function postgrado_nuevo() {
        ...
        if ($this->input->post('sin_postgrado')) {
            redirect('epostulante/superior_nuevo');
        } else {
...
//                para actulizar la fecha de uno de los tab
    ...
    function superior_nuevo() {
        ...
        if ($this->input->post('sin_superior')) {
            redirect('epostulante/secundaria_nuevo');
        } else {
...
//    }
    ...
    function idioma_nuevo() {
        ...
        if ($this->input->post('sin_idioma')) {
            redirect('epostulante/informacion_adicional');
        } else {
...
//                $idDisponible[$prefijo . 'salario'] = $this->input->post($prefijo . 'salario');
            ...
            // $datos['con_pretension_salarial'] = $this->input->post($this->prefijoF . 'salario');
            // $datos['contador_id'] = $this->input->post('contador');
            // // $datos[$this->prefijo . 'fecha_creacion'] = date('Y-m-d H:i:s');
            // // $datos[$this->prefijo . 'fecha_edicion'] = date('Y-m-d H:i:s');
            // $this->db->insert('convocatoria_postulacion', $datos);
            // var_dump($this->input->post($this->prefijoF . 'salario'));
            // exit();
            redirect('epostulante/editar_cv/id/' . $_SESSION[$this->presession . 'id']);
//            redirect('newpostulante/instruccion_formal');
    ...
    public function guarda_salario()
    {
        $salario = $this->input->post('param1');
        $contador = $this->input->post('param2');
        $disponibilidad = $this->input->post('param3'); 
        ...
        $respuesta = array('estado'=>'registrado', 'salario' => $_SESSION[$this->presession . 'salario'], 'contador' => $_SESSION[$this->presession . 'contador'], 'disponibilidad' => $_SESSION[$this->presession . 'disponibilidad']);
    ...
    {
        // var_dump($this->input->get('param1'));
        // exit;
        // $datos = array(
        ...
        $num = $this->input->post('salario');
        $respuesta = array('estado'=> $num);
        echo json_encode($respuesta);
        // return $respuesta;
    }
