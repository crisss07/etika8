<?php
if (@$this->opciones['nuevo']) {
    $nuevo = $this->opciones['nuevo'];
} else {
    $nuevo = 'Nuevo';
}
if (@$this->opciones['listado']) {
    $listado = $this->opciones['listado'];
} else {
    $listado = 'Listado';
}
        ...
        <tr>
            <td class="enlaces_add_edit" align="left" width="100%">
                ...
                if ($this->idp) {
                    ...
                    if (@$this->fid) {
                        ?>
                        <?php echo anchor($this->controlador . 'agregar/fid/' . $this->fid . '/idp/' . $this->idp, $nuevo, array('class' => 'enlace_nuevo enlace_a1')); ?>&nbsp;&nbsp;
                        <?php echo anchor($this->controlador . 'listar/fid/' . $this->fid . '/idp/' . $this->idp, $listado, array('class' => 'enlace_listar enlace_a1')); ?>
                        &nbsp;&nbsp;
                        <?php echo anchor($this->controlador . 'listar/fid/' . $this->fid . '/idp/' . $this->idp, 'Cancelar', array('class' => 'enlace_cancelar enlace_a1')); ?>
                        <?php
                    } else {
                        ?>
