...
//        $this->sector_experiencia[-1] = 'Seleccione el Sector de Experiencia';
    ...
    function listar() {
        ...
        if (@$cadena || @$cliente || @$cargo || @$instancia || @$recomendacion) {
            ...
            $this->campos_listar = array('Apellido Paterno', 'Apellido Materno', 'Nombres', 'Documento', 'País', 'Celular', 'Email','Observación','Recomendación', 'Estado');
            $this->campos_reales = array($prefijo . 'apaterno', $prefijo . 'amaterno', $prefijo . 'nombre', $prefijo . 'documento', 'pais', $prefijo . 'celular', $prefijo . 'email',$prefijo . 'observacion', $this->prefijoF . 'recomendacion', $this->prefijoF . 'estado');
            $this->hiddens = array($this->prefijo . 'id');
...
//            ORDER BY pos_apaterno asc, pos_tipodoc asc, pos_documento asc';
    ...
    function zip_hello_world($idc = null) {
        $zip = new ZipArchive;
        $tmp_file='./archivos/test.zip';
if ($zip->open($tmp_file,ZipArchive::CREATE) === TRUE) {
    $zip->addFile('./archivos/hola.txt', 'newname.txt');
    $zip->close();
    echo 'ok';
    ...
    header("Content-Disposition: attachment; filename=hola.zip");
