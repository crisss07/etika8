...
{  
    ...
    {        
        $sw=0;
        if(@$this->definirform){$this->definir_form_editar();$sw=1;}
        else{$this->definir_form_agregar();$sw=2;}
            ...
            for($i=0;$i<count(array(@$this->campoup_img));$i++)
             {
                ...
                $fila[$this->prefijo.'img_borrar'.$j]=@set_value($fila1[@$this->campoup_img[$i]]);                
                $fila[@$this->campoup_img[$i]]='';
